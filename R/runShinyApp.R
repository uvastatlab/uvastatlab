...
runShinyApp <- function(app) {
  ...
  validExamples <- list.files(system.file("shiny-apps", package = "uvastatlab"))
  ...
  appDir <- system.file("shiny-apps", app, package = "uvastatlab")
  if (appDir == "") {
    stop("Could not find example directory. Try re-installing `uvastatlab`.", call. = FALSE)
  }

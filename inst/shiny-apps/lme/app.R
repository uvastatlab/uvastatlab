...
ui <- fluidPage(
   ...
   sidebarLayout(
      sidebarPanel(
        ...
        h3("Fixed Effects"),
        sliderInput("a","Intercept coefficient:",
                    min = -5,max = 5,value = 0,step = 0.1),
        sliderInput("b","Slope coefficient:",
                    min = -5,max = 5,value = 0,step = 0.1),
        ...
        conditionalPanel(
          ...
          sliderInput("cor", "Intercept and Slope correlation",
      ...
      mainPanel(
        p('Set the parameters of the data generating process using the controls
        in the left panel. See how well', code('lmer'), 'estimates the
        parameters in the model output and accompanying graph. The thick
        black line is the fitted fixed effect. The colored lines represent the
        subject-specific fitted lines.'),
        plotOutput("lmePlot"),
